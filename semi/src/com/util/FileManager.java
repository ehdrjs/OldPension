...
public class FileManager {
	...
	public static boolean doFiledownload(String saveFilename, String originalFilename, String pathname, HttpServletResponse resp) {
		...
		try {
			originalFilename=new String(originalFilename.getBytes("euc-kr"), "8859_1"); //서버가 클라이언트에게 파일을 전송할 때 스트림을 쏜다.
					//스트림을 전송하기 위해서 "application/octet-stream"으로 setting해준다.
					//8859-1(web browser가 알아서 바꿔준다.)로 바꿔줘야 한다.
			...
			// 클라이언트에게 전송할 출력 스트림(PrintStream은 인코딩 된 것이므로 사용하게 되면 깨진다.), OutputStream을 사용해서 클라이언트에게 보내야한다.ㄴ
			...
			os.flush();	
			os.close();	//write시 닫아줘야한다.
			bis.close();//write시 닫아줘야한다.
