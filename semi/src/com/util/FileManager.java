...
public class FileManager {
	...
	public static boolean doFiledownload(String saveFilename, String originalFilename, String pathname, HttpServletResponse resp) {
		...
		try {
			originalFilename=new String(
					originalFilename.getBytes("euc-kr"), "8859_1"); //서버환경에따라 euc-kr을 수정한다
			...
			resp.setContentType("application/octet-stream"); //파일은 스트림으로 쏴야한다
			...
			BufferedInputStream bis=
					new BufferedInputStream(
							new FileInputStream(f));  //f는 내 파일명
			
			// 클라이언트에게 전송할 출력 스트림(클라이언트에게 전송)
	...
	public static String doFilerename(String pathname, String filename) {
    	...
    	String s = String.format(//년월일시분나노초로 중복 회피
