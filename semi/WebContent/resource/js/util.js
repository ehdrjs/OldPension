...
function numberWithCommas(n) {
    var parts = n.toString().split("."); // 소숫점기준으로 split // [0]번째 정수파트[1]번째 실수파트
    parts[0] = parts[0].replace(/\B(?=(\d{3})+(?!\d))/g, ","); // ,로 replace
    // \B:단어사이 / 숫자3개 /  앞문자 0 또는 1개(:?) / 앞문자와 1개이상의 문자(:+)
    // 12AB12 : \B // AB12 : \b
    // (?!\d) : 숫자가 아니다.
    return parts.join("."); // 소숫점으로 결합 (기본은 컴마)
}

// 컴마 제거
function removeComma(str) {
    str = String(str); // 숫자면 문자로
   // return str.replace(/[^\d]+/g, '');
    return str.replace(/[,]/g, ''); // g는 모든 컴마 제거
}
...
//*************************************
// 주민등록 번호 검사
function isValidResidentNO(ssn1, ssn2) { // (생년월일, 번호)
        ...
        chkNum = 11 - tot % 11;
        chkNum = chkNum % 10; // chkNum 숫자가 주민번호 마지막 숫자와 동일해야한다.
